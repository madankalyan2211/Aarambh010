...
// Test Firebase environment
router.get('/test-firebase-env', async (req, res) => {
  try {
    // Check if required environment variables are present
    const requiredEnvVars = [
      'FIREBASE_PROJECT_ID',
    ...
    // Also check if Firebase Admin SDK can be initialized
    let firebaseInitSuccess = false;
    let firebaseInitError = null;
    
    try {
      const admin = require('firebase-admin');
      if (allPresent && !admin.apps.length) {
        // Try to initialize
        admin.initializeApp({
      ...
      firebaseInitError = initError.message;
    }
    ...
    console.error('Firebase env test error:', error);
    res.status(500).json({
      success: false,
      message: 'Error testing Firebase environment',
      error: error.message
    });
  }
});
